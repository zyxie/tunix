...
class vLLMSamplerTest(absltest.TestCase):
  ...
  def test_vllm_sampler(self):
      ...
      _vllm_sampler.load_checkpoint(state)
      ...
      self.assertEqual(vanilla_output.text, ["Nice to meet you. What's your name?", 'Paris.'])

      print("-" * 50)
      print(f"vLLM Generated text: {vllm_output.text}")
      self.assertEqual(vllm_output.text, ["It's nice to meet you. Is there something I can help you with or would you like to chat?", 'The capital of France is Paris.'])

      _, tunix_state = nnx.split(tunix_model)
      vllm_state = _vllm_sampler._model_runner.state
      if os.environ.get("NEW_MODEL_DESIGN") == "True":
        self.assertTrue(
          np.allclose(tunix_state["lm_head"]["w"].value,
                      vllm_state["lm_head"]["input_embedding_table_DV"].value))
      else:
        self.assertTrue(
          np.allclose(tunix_state["lm_head"]["w"].value,
                      vllm_state["lm_head"].value))
