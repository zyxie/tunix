...
def _normalize_device_kind(device_kind: str) -> str | None:
  ...
  Args:
    device_kind: The runtime device kind, e.g. ``"TPU v5 lite"`` or ``"TPU v7"``.
...
def _device_host_key(device: Any) -> tuple[Any, ...] | None:
  ...
  Returns ``None`` when no task metadata is available, which callers treat as
  "host grouping unknown" and fall back accordingly.
  """
...
def best_topology_shapes_for_chip_count(
  ...
  single supported shape for the count, projected to the requested ``chip_rank``.
  ...
  Examples:
    Edge family, 2D vs 3D projection::
        ...
        best_topology_shapes_for_chip_count("TPU v6e", 8, chip_rank=2)  # [(2, 4)]
        best_topology_shapes_for_chip_count("TPU v6e", 8, chip_rank=3)  # [(2, 4, 1)]

    Fish family picks the most cubical arrangement, and honors the bound::

        best_topology_shapes_for_chip_count("TPU v7", 256)              # [(4, 8, 8)]
        best_topology_shapes_for_chip_count(
            "TPU v7", 576, available_chip_shape=(4, 12, 16))            # [(4, 12, 12)]
  """
  if required_chips <= 0:
    return []
